...
public class WhatsappWebhookService implements WhatsappWebhookUseCase {
    ...
    public Optional<String> verifyWebhook(String mode, String providedVerifyToken, String challenge) {
        ...
        if (verifyToken == null || verifyToken.isBlank() || !verifyToken.equals(providedVerifyToken)) {
            return Optional.empty();
        }
    ...
    private void processIncomingTextMessage(JsonNode message) {
        ...
        String text = trimToNull(message.path("text").path("body").asText());
        if (waId == null || text == null) {
            return;
        }

        OffsetDateTime now = OffsetDateTime.now();
        if (!whatsappLinkPort.markInboundMessageIfNew(metaMessageId, waId, now)) {
            return;
        }
        ...
        if (linked.isPresent()) {
            ...
            if (user == null) {
                whatsappMessagePort.sendTextMessage(waId, "No pude identificar tu cuenta de Trackademy. Vuelve a vincular tu numero desde la web.");
                return;
            }

            String reply = whatsappCommandService.resolveCommand(user.email(), text);
            whatsappMessagePort.sendTextMessage(waId, reply);
            return;
        }
        ...
        whatsappMessagePort.sendTextMessage(
                waId,
    ...
    private void handleLinkCode(String waId, String code, OffsetDateTime now) {
        ...
        if (codeOpt.isEmpty()) {
            whatsappMessagePort.sendTextMessage(waId, "No reconoci ese codigo. Genera uno nuevo desde tu cuenta de Trackademy e intentalo otra vez.");
            return;
        }
        ...
        if (linkCode.expiresAt() == null || !linkCode.expiresAt().isAfter(now)) {
            ...
            whatsappMessagePort.sendTextMessage(waId, "Este codigo de vinculacion ya expiro. Genera uno nuevo desde tu cuenta de Trackademy.");
            return;
        }
        ...
        if (existingByWaId.isPresent() && !existingByWaId.get().userId().equals(linkCode.userId())) {
            whatsappMessagePort.sendTextMessage(waId, "Este numero ya esta vinculado a otra cuenta de Trackademy. Revisa tu configuracion desde la web.");
            return;
        }
